class ParkingSlotReservation < ApplicationRecord
  ...
  def self.get_histories(park, type, from, to)
    ...
    monthly = hash[type]
    if park == '0'
      where.not(park_id: nil).where(is_monthly: monthly, timeout: from..to.to_datetime.end_of_day)
    else
      where(park_id: park, is_monthly: monthly, timeout: from..to.to_datetime.end_of_day)
    end
  end
