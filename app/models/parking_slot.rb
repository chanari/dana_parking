class ParkingSlot < ApplicationRecord
  ...
  def reserve(number_plate, client)
    return false if User.where(id: client, role: '0').empty? || ParkingSlot.find_by(client: client, number_plate: number_plate).present?
    SlotExpiredJob.set(wait: 50.minutes).perform_later(self.id, false) if self.update(status: '1', date_in: DateTime.now, number_plate: number_plate, client: client)
  end
